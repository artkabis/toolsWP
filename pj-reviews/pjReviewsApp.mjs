...
import * as jQuery from 'https://cdnjs.cloudflare.com/ajax/libs/jquery/3.6.0/jquery.min.js';
...
export const pjReviewsApp = async ({idClient='',nbReviews=null,containerReviews=''}) =>{
  ...
  if ($(containerReviews).length && idClient && containerReviews) {
    ...
    $('#PJ-review-container').before(`<a class="new-review pj-link" href="https://www.pagesjaunes.fr/contribution/avis/${IDPRODUCT}" target="_blank"><span class="fa fa-pencil"></span><span class="value">Ecrire un avis</span></a>`);//Add reviews button for adding a new review to pj pro

    const api_call = 'https://api.pagesjaunes.fr/v1/pros/' + IDPRODUCT + '/reviews';//Create url for api call
