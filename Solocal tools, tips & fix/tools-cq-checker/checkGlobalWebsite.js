javascript:(($)=>{
  ...
  title && title.length > 0 ? console.log(`%c Meta title : ${title} -> caractère : ${title.length} ----- (de 50 à 65)`, `color:${title.length >= 50 && title.length <= 65 ? "green" : "red"}`) : console.log(`%c Meta title non présent !!!`, `color:red`);
  desc && desc.length > 0 ? console.log(`%c Meta description : ${desc} -> caractère : ${desc.length} ----- (de 140 à 156)`, `color:${desc.length >= 140 && desc.length <= 156 ? "green" : "red"}`) : console.log(`%c Meta desc non présente !!!`, `color:red`);
  console.log("----------------------------- END Check META --------------------------------------------");
  console.log("----------------------------- Check ALT images --------------------------------------------");
  $("img").each(function(i, t) {
      const src = ($(this).attr("src")) ? $(this).attr("src") : $(this).attr("data-src");
      if (src && !src.includes("mappy") && !src.includes('cdn.manager.solocal.com')) {
  ...
  document.querySelectorAll("h1,h2,h3,h4,h5,h6").forEach((t,i)=>{
      ...
      if (((tagName === "H1" || tagName === "H2") && nbLetters < 50) || nbLetters > 90) {
          console.log("%c" + tagName + " : " + tagContent + " ------ Erreur -> nombre de caractères : " + nbLetters + ", ne rentre pas dans la préco de 50 -> 90 caractères", "color:red");
      }
  }
  );
  ...
  const checkerImageWP = ()=>{
      if ($("#Content").length) {
          ...
          $("img").each(function(t, i) {
              $(this) && $(this).attr("src") && !$(this).attr("src").includes("mappy") && !$(this).attr("src").includes('cdn.manager.solocal.com') && checkUrlImg($(this), $(this)[0].src, $(this)[0].getAttribute("alt"), $(this)[0].getAttribute("title"), "srcImage", $(this)[0].naturalWidth, $(this)[0].naturalHeight, $(this)[0].parentNode.offsetWidth, $(this)[0].parentNode.offsetHeight, );
          });
                  ...
                  if (bgimg && !bgimg.includes("undefined")) {
                      customImg.onload = function() {
                          if (!bgimg.includes("mappy") && !bgimg.includes('cdn.manager.solocal.com')) {
                              !bgimg.includes("data:image/") ? checkUrlImg($(this), bgimg, "no alt -> gbimg", "no title -> gbimg", "bgImage", customImg.naturalWidth, customImg.naturalHeight, _this[0].parentNode.offsetWidth, _this[0].parentNode.offsetHeight, ) : console.log("base64 img detected : ", bgimg.includes("data:image/"), " width : ", customImg.width, " height : ", customImg.height, " url : ", bgimg);
                          }
                      }
                      ;
                  }
      ...
      return new Promise(function(resolve, reject) {
          ...
          xhr.onreadystatechange = function(data) {
              if (xhr.readyState == 4) {
                  ...
                  resolve(response);
                  response.status !== 404 ? console.log(`%c url : ${url} status : ${response.status}`, "color:green") : console.log(`%c url : ${url} status : ${response.status}`, "color:red");
              }
          }
          ;
          try {
  ...
  document.querySelectorAll("a").forEach(function(t, i) {
      ...
      if (url) {
          ...
          const verif = !url.includes("tel:") && !url.includes("mailto:") && !url.includes("javascript:") && !url.includes("logflare") && !url.includes("solocal") && !url.includes("sp.report-uri") && !url.includes("chrome-extension") && !url.includes("pagesjaunes") && url.at(0) !== '#';
          (verif && url.includes(window.location.origin)) && check(url);
          (verif && !url.includes(window.location.origin)) && (console.log(`%c Vérifier manuellement ce lien ${url}`, 'color:red'),
          console.log(t));
      }
  });
  setTimeout(function() {
