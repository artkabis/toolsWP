...
/******* testing width and height background-image inner and outer ->>>>>>>>
jaavascript:(() => {
  function formatBytes(bytes) {
    if (bytes < 1024) return bytes + " Bytes";
    else if (bytes < 1048576) return (bytes / 1024).toFixed(2) + " KB";
  ...
  function checkUrlImg($this,url, alt, title, type, width, height, parentWidth, parentHeight) {
    ...
    req.onreadystatechange = function () {
      if (this.readyState == this.HEADERS_RECEIVED) {
        var headers = req.getAllResponseHeaders();
          console.log('_________ headers ::::::',headers);
        var arr = headers.trim().split(/[\r\n]+/);
        var map = {};
        arr.forEach(function (line) {
          var parts = line.split(": ");
        ...
        if (fsize != null) {
          var result = {
            ...
            if(type=== 'srcImage'){
                ...
                result.Imgwidth=$this.innerWidth(),
                result.Imgheight=$this.innerHeight(),
                result.parentwidth=$this.parent().width(),
                result.parentheight=$this.parent().height()
            }
    ...
    (function($) {
        document.querySelectorAll('img').forEach(function(t, i) {
            (t && t.src) && checkUrlImg($(t),t.src, t.getAttribute('alt'), t.getAttribute('title'), 'srcImage',t.width,t.height, t.parentElement.clientWidth, t.parentElement.clientHeight);
        });
        document.querySelectorAll('div').forEach(function(t, i) {
            if (t && t.style.backgroundImage && String(t.style.backgroundImage).includes('url(')) {
                let bgimg = t.style.backgroundImage.split('url("')[1].split('")')[0];
                bgimg = (bgimg.includes('http')) ? bgimg : window.location.origin + bgimg;
                (bgimg) && checkUrlImg($(this),bgimg, 'no alt -> gbimg', 'no title -> gbimg', 'bgImage');
            }
        });
        const element = $('div[class*="vc_custom_"]');
        element.each(function(i, t) {
