...
const max_img = 25, start_img = 5;//params de délimitation
function pause(msec) {
    return new Promise(
        (resolve, reject) => {
...
function convertirHrefEnBlobEtTelecharger(href,name,cmp) {
    ...
    console.log('loop<=max_img : ',loop<=max_img,'    loop >= start_img : ',loop >= start_img,'     href.includes("site-privilege") :',String(href).includes('site-privilege'));
...
async function downloadAll(elements) {
    ...
    for (var e in elements) {
        convertirHrefEnBlobEtTelecharger(elements[e].img,elements[e].name, elements[e].cmp); // Lancement de la construction des img et du download.
        //Gestion de la pause de 1 seconde toutes les 10 itérations
        if (++count >= 10) {
            await pause(1000);
            count = 0;
        }
